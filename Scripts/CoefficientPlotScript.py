...
N   = 3
lam = 1
R   = 100.0   # string length (history length used in payoff denominators)
...
def DY_val(q, p, rho=None):
    """Second-order payoff curvature term in the Y direction."""
    if rho is None:
        rho = p
    t = _inner(q, p, rho)
...
def DX_val(q, p, rho=None):
    """Second-order payoff curvature term in the X direction."""
    if rho is None:
        rho = p
    inner_a = (
        p ** N * (q - N * q + p * (p + (-2 + N) * q + p * (-1 + q) * rho))
...
def AX_val(q, p, rho=None):
    """Linear-in-q payoff coefficient A_X."""
    if rho is None:
        rho = p
    return (
        q + (-1 + p ** (-2 + N)) * q
...
def BX_val(q, p):
    """Constant payoff coefficient B_X."""
    return (
...
# Full X and Y payoff components 
...
def q_star(p):
    """Solve for the symmetric equilibrium mixing probability at belief p."""
    X = X_full(0.5, p)
    Y = Y_full(0.5, p)
    if abs(Y) < 1e-12:
        return np.nan
    return np.clip(-X / (2 * Y), 0, 1)


#Analytical curvature coefficient P2

def P2_val(p):
    """Compute the curvature coefficient P2(rho*) at the equilibrium."""
    qs = q_star(p)
    if np.isnan(qs):
        return np.nan
    AX = AX_val(qs, p)
...
def full_coeff(p):
    """Return (P2, 3*P2 / (4*lambda*D_Y)) at the equilibrium strategy."""
    qs = q_star(p)
    if np.isnan(qs):
        return np.nan, np.nan
    p2 = P2_val(p)
    dy = DY_val(qs, p)
    return p2, 3 * p2 / (4 * lam * dy)


# ── Plotting ──────────────────────────────────────────────────────────────────

def plot_curvature(rho_vals, P2_vals, coeff_vals,
                   save_path="equilibrium_curvature1.png"):
    """
    Plot P2(rho*) and the full payoff coefficient 3*P2/(4*lambda*D_Y).
    Saves to save_path at 300 dpi.
    """
    COLOR_P2    = '#2E86AB'
    COLOR_COEFF = '#E84855'
    COLOR_ZERO  = '#6B6B6B'
    ...
    ax1.set_ylabel(r'$\frac{3P_2}{4\lambda D_Y}$', fontsize=12)
    ax1.set_title(r'Payoff coefficient $\frac{3P_2}{4\lambda D_Y}$',
                  fontsize=13, pad=10)
...
def main():
    ...
    P2_vals    = np.array([full_coeff(r)[0] for r in rho_vals])
    coeff_vals = np.array([full_coeff(r)[1] for r in rho_vals])
